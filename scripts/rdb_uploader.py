...
def main() -> None:
    ...
    if args.sign_only:
        ...
        # 4. Upload OOM dump files (best-effort, non-fatal — generated by OOM automation at 70%/80%/90%)
        #    Done before persistence validation so dumps are captured even if RDB/AOF upload failed.
        print("\n[5/7] Checking for OOM dump files...")
        oom_dump_files = ["oom_dump_70.log", "oom_dump_80.log", "oom_dump_90.log"]
        oom_dump_urls = []
        for dump_file in oom_dump_files:
            pod_path = f"/data/{dump_file}"
            if kubectl_check_path(args.namespace, args.pod, args.container, pod_path):
                print(f"  {pod_path} — found, uploading...")
                try:
                    dump_object = f"{args.namespace}/{dump_file}"
                    dump_blob = client.bucket(args.bucket).blob(dump_object)
                    dump_put_url = get_signed_url(dump_blob, creds, 60, method="PUT")
                    mask_in_actions(dump_put_url)
                    kubectl_exec(
                        args.namespace, args.pod, args.container,
                        [
                            "curl", "-X", "PUT", "--fail", "--silent", "--show-error",
                            "-H", "Content-Type: text/plain",
                            "--upload-file", pod_path,
                            dump_put_url,
                        ],
                        redact_args=[dump_put_url],
                    )
                    oom_dump_urls.append(f"gs://{args.bucket}/{dump_object}")
                    print(f"  {dump_file} uploaded successfully.")
                except Exception as e:
                    print(f"  ⚠️  {dump_file} upload failed (non-fatal): {e}")
            else:
                print(f"  {pod_path} — not found (threshold not reached), skipping.")

        # 5. Verify at least one persistence artifact was uploaded
        print("\n[6/7] Validating artifact uploads...")
        ...
        # 6. Output GCS paths (IAM authentication required to download)
        print("\n[7/7] Outputting GCS paths...")
        if rdb_exists:
            write_github_output("rdb_url", rdb_gcs_path)
        else:
            write_github_output("rdb_upload_skipped", "true")
