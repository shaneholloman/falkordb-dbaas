...
const SECURITY_INFRA_POOL_NAME = 'secinfra';
...
// ── Security-Infra Node Pool Delete (BYOA) ─────────────────────────────────

export async function deleteSecurityInfraNodePoolGCPBYOA(cluster: Cluster): Promise<void> {
  try {
    if (!cluster.gcpAccountID || !cluster.gcpAccountNumber) {
      logger.error({ cluster: cluster.name }, 'Missing gcpAccountID or gcpAccountNumber for BYOA cluster');
      return;
    ...
    const exists = nodePools.nodePools?.some((np) => np.name === 'security-infra');
    ...
    logger.info({ cluster: cluster.name }, 'Security-infra node pool deleted for BYOA GCP cluster.');
  } catch (error) {
    logger.error(
      { cluster: cluster.name, errorName: (error as any)?.name, errorMessage: (error as any)?.message, errorDetails: error },
      'Failed to delete security-infra node pool for BYOA GCP cluster',
    );
    throw error;
  }
}

export async function deleteSecurityInfraNodePoolAWSBYOA(cluster: Cluster): Promise<void> {
  try {
    if (!cluster.awsAccountID || !cluster.awsRoleARN) {
      logger.error({ cluster: cluster.name }, 'Missing AWS credentials for BYOA cluster');
      return;
    ...
    logger.info({ cluster: cluster.name }, 'Security-infra node pool deletion initiated for BYOA AWS cluster.');
  } catch (error) {
    logger.error(
      { cluster: cluster.name, errorName: (error as any)?.name, errorMessage: (error as any)?.message, errorDetails: error },
      'Failed to delete security-infra node pool for BYOA AWS cluster',
    );
    throw error;
  }
}

export async function deleteSecurityInfraNodePoolAzureBYOA(cluster: Cluster): Promise<void> {
  try {
    if (!cluster.azureClientId || !cluster.azureTenantId || !cluster.azureResourceGroupName) {
      logger.error({ cluster: cluster.name }, 'Missing Azure credentials for BYOA Azure cluster');
      return;
    ...
    try {
      await client.agentPools.get(cluster.azureResourceGroupName, cluster.name, SECURITY_INFRA_POOL_NAME);
    } catch (error: any) {
      if (error.statusCode === 404 || error.code === 'ResourceNotFound' || error.code === 'AgentPoolNotFound') {
        logger.info({ cluster: cluster.name }, 'Security-infra node pool does not exist, nothing to delete.');
        return;
    ...
    logger.info({ cluster: cluster.name }, 'Security-infra node pool deleted for BYOA Azure cluster.');
  } catch (error) {
    logger.error(
      { cluster: cluster.name, errorName: (error as any)?.name, errorMessage: (error as any)?.message, errorDetails: error },
      'Failed to delete security-infra node pool for BYOA Azure cluster',
    );
    throw error;
  }
}
