...
export async function deleteSecurityInfraNodePool(cluster: Cluster): Promise<void> {
  const parent = `projects/${process.env.APPLICATION_PLANE_GOOGLE_CLOUD_PROJECT}/locations/${cluster.region}/clusters/${cluster.name}`;
  let exists = false;
  try {
    const [nodePools] = await client.listNodePools({ parent });
    exists = nodePools.nodePools?.some((np) => np.name === 'security-infra');
  } catch (error) {
    if (error.code === 5) {
      logger.warn({ cluster: cluster.name }, 'Security-infra node pool not found, already deleted.');
      return;
    }
    logger.error({ cluster: cluster.name, error }, 'Failed to list security-infra node pools');
    throw error;
  }
  ...
  try {
    ...
    logger.info({ cluster: cluster.name }, 'Security-infra node pool deleted.');
  } catch (error: any) {
    logger.error({ cluster: cluster.name, error }, 'Failed to delete security-infra node pool');
    throw error;
  }
}
