...
describe('RDB export copy RDB to bucket processor', () => {
  ...
  it('does not complete default pod upload copies before the read URL job runs', async () => {
    const tasksRepository = {
      getTaskById: jest.fn().mockResolvedValue({
        ...makeDefaultMultiShardTask(),
    ...
    const k8sRepository = {
      sendUploadCommand: jest.fn().mockResolvedValue(undefined),
    };
    ...
    expect(k8sRepository.sendUploadCommand).toHaveBeenCalledWith(
      'gcp',
      'cluster-id',
      'us-central1',
      'instance-id',
